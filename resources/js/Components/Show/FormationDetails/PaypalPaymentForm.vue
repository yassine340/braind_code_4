<template>
  <div class="space-y-4">
    ...
    <div class="p-4 border border-gray-300 rounded bg-gray-50 text-center">
      ...
      <p class="text-sm text-gray-500">Le montant à payer sera débité de votre compte PayPal.</p>
    </div>
    
    <button 
      @click="$emit('redirect-to-paypal')" 
      :disabled="paymentProcessing || !paypalApprovalUrl || !isFormValid" 
      class="w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded"
      :class="{ 'opacity-50 cursor-not-allowed': paymentProcessing || !paypalApprovalUrl || !isFormValid }"
    >
...
const emit = defineEmits<{
  (e: 'redirect-to-paypal'): void;
}>();

// Validate that the form is complete before allowing to proceed
const isFormValid = computed(() => {
  return props.paymentInfo.address && 
         props.paymentInfo.postal_code && 
         props.paymentInfo.city && 
         props.paymentInfo.country;
});
</script>